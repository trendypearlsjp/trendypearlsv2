...
app.post('/api/publish', (req, res) => {
    exec('git --version', (gitCheckErr) => {
        ...
        exec('git status --porcelain', { cwd: __dirname }, (statusErr, stdout) => {
            ...
            const syncCmd = `git pull --rebase origin main ; git push origin main && git push origin main:master`;
            ...
            exec(syncCmd, { cwd: __dirname }, (pushErr, pushStdout, pushStderr) => {
                if (pushErr) {
                    ...
                    const forceCmd = `git push -f origin main && git push -f origin main:master`;
