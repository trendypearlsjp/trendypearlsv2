...
// Helper function to auto pull latest updates from GitHub
function autoPullFromGithub(callback) {
    exec('git --version', (gitCheckErr) => {
        if (gitCheckErr) {
            if (callback) callback({ success: false, error: 'Git not installed' });
            return;
        }
        ...
        exec('git pull origin main', { cwd: __dirname }, (err, stdout, stderr) => {
            if (err) {
                console.warn('Auto-pull notice:', stderr || err.message);
                if (callback) callback({ success: false, error: stderr || err.message, output: stdout });
            } else {
                console.log('Auto-pulled latest updates from GitHub successfully!');
...
app.get(['/admin', '/admin/', '/admin.html', '/dashboard'], (req, res) => {
    // Automatically trigger git pull in background when Admin panel is accessed
    autoPullFromGithub();
    res.sendFile(path.join(__dirname, 'admin.html'));
});

// API Endpoint to explicitly trigger Auto-Pull
app.all('/api/pull', (req, res) => {
    autoPullFromGithub((result) => {
        res.json(result);
    });
});
...
function startServer(p) {
    const server = app.listen(p, () => {
        ...
        console.log(`===================================================`);
        // Automatically pull latest updates on server startup
        autoPullFromGithub();
    });
