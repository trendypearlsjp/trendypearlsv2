...
function applyFiltersAndSort() {
    try {
        ...
        // Identify the newest 50 products in the store for the FRESH badge
        ...
        filteredCatalog = sortedSource.filter(item => {
            ...
            let matchCat = false;
            if (activeCategory === "all" || activeCategory === "" || activeCategory === "all products") {
                matchCat = true;
            ...
            // Jewellery Subcategory Check (Only applied on Jewellery page)
...
function renderCardHTML(item) {
    ...
    const freshBadge = isFresh 
        ? `<span class="absolute top-2.5 left-2.5 z-20 bg-[#9E6B7B]/85 text-white font-bold text-[9px] px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow-xs backdrop-blur-xs">FRESH</span>` 
        : '';
