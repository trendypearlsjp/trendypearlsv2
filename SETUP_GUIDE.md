...
## 5. Automatic Auto-Pull & Update System (`auto_update/`)

When team members make major changes or update product catalogs from another laptop:
- **Opening Admin Section**: Navigating to `http://localhost:3000/admin` automatically runs `git pull origin main` in the background.
- **Starting Local Server**: Double-clicking **`run_localhost.bat`** automatically pulls all latest updates and details before launching the server.
- **Manual 1-Click Pull**: Click **`PULL LATEST UPDATES`** inside the Admin Panel header or double-click **`auto_update/pull_latest.bat`**.

You **never need to uninstall or reinstall** the app when big changes occur!
